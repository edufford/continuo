...
/// because a component cannot be reconstructed from bytes. Replaying a
/// dynamic run means re-running the scenario that issues the same joins and
/// leaves, and checking the resulting stream against the log.
